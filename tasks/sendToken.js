...
module.exports = async function (taskArgs, hre) {
    ...
    let minGasToTransferAndStore = await onft.minGasToTransferAndStore();
    let transferGasPerToken = await onft.dstChainIdToTransferGas(remoteChainId)
    console.log(remoteChainId, minGasToTransferAndStore);
    ...
    let fees = await onft.estimateSendFee(remoteChainId, toAddress, taskArgs.tokenId, false, adapterParams)
    ...
    try {
        let tx = await (
            await onft.sendFrom(
                ...
                taskArgs.tokenId,                       // tokenIds to send
