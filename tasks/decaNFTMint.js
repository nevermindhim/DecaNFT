module.exports = async function (taskArgs, hre) {
    let contract = await ethers.getContract(taskArgs.contract)

    try {
        if (await contract.mintState() == false)
            await (await contract.setMintState(true)).wait()
        let tx = await (await contract.mint(taskArgs.qty)).wait()
        console.log(`✅ [${hre.network.name}] mint()`)
        console.log(` tx: ${tx.transactionHash}`)
        //let decaTokenId = await ethers.provider.getTransactionReceipt(tx.transactionHash)
        //console.log(` ONFT nftId: ${parseInt(Number(decaTokenId.logs[0].topics[3]))}`)
    } catch (e) {
        if (e.error?.message.includes("ONFT: Max limit reached")) {
            console.log("*ONFT: Max limit reached*")
        } else {
            console.log(e)
        }
    }
}