...
describe("SpheraKitBag", function () {
    ...
    beforeEach(async function () {
        ...
        spheraKitBag = await SpheraKitBag.deploy("SpheraKitBag", "SKB", 1000, owner.address)
    ...
    describe("Period Management", function () {
        ...
        it("should allow the owner to set period parameters", async function () {
            ...
            await spheraKitBag.setPeriodParams(
                1,
        ...
        it("should not allow non-owner to set period parameters", async function () {
            ...
            await expect(
                spheraKitBag
                    .connect(addr1)
                    .setPeriodParams(1, Math.floor(Date.now() / 1000), Math.floor(Date.now() / 1000) + 60 * 60, 1000000000000000, 10, 100)
        ...
        it("should revert if trying to mint more than the allowed amount in a period", async function () {
            ...
            await expect(spheraKitBag.periodMint(1, 11, proof, { value: ethers.utils.parseEther("0.05") })).to.be.revertedWith(
                "MintingTooMuchInPeriod()"
            )
        })
        ...
        it("should revert if not enough Ether is sent to cover the minting cost", async function () {
            ...
            await expect(spheraKitBag.periodMint(1, 1, proof, { value: ethers.utils.parseEther("0.0001") })).to.be.revertedWith(
                "InsufficientFunds()"
            )
        })
        ...
        it("should revert if trying to mint beyond the max supply for a period", async function () {
            ...
            await expect(
                spheraKitBag.connect(addr1).periodMint(1, 1, proofaddr1, { value: ethers.utils.parseEther("0.05") })
            ).to.be.revertedWith("MaxPeriodMintSupplyReached()")
        })
        ...
        it("should revert if trying to mint before the period start time", async function () {
            ...
            await spheraKitBag.setPeriodParams(
                1,
                Math.floor(Date.now() / 1000) + 60 * 60,
                Math.floor(Date.now() / 1000) + 90 * 60,
                1000000000000000,
                10,
                100
            )
        ...
        it("should revert if trying to mint in a period that does not exist", async function () {
            ...
            await expect(spheraKitBag.periodMint(999, 1, proof, { value: ethers.utils.parseEther("0.05") })).to.be.revertedWith(
                "PeriodNotOpen()"
            )
        })
    })
